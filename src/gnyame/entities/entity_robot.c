...
void gny_entity_robot_on_render(NYA_Entity* entity, NYA_Window* window) {
    ...
    for (u32 i = 0; i < GNY_ROBOT_DRONES; i++) {
        ...
        f32x2         position = nya_entity_render_position(entity).xy;

        nya_render2d_line(window, position, robots->waypoints[i], 1.0F, (NYA_Color){ color.r, color.g, color.b, 0.35F });
        ...
        nya_render2d_triangle(window, position + forward, position - (forward * 0.6F) + side, position - (forward * 0.6F) - side, color);
        return;
    }
}
