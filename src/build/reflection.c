...
void nya_reflection_generate(void) {
    ...
    nya_info("nya_reflection_generate: %u annotated types across %llu headers.", set.type_count,
...
NYA_ConstCString _nya_reflect_hint_from_comment(const NYA_Lexer* lexer, u32 index) {
    ...
    nya_warn("nya_reflection_generate: unknown @hint(%s), treated as none.", argument);
...
void _nya_reflect_scan_file(_NYA_ReflectSet* set, NYA_ConstCString path) {
    ...
    for (u32 index = 0; index < lexer.tokens->length; index++) {
        ...
        if (_nya_reflect_token_is(&lexer, cursor, "struct")) {
            ...
            nya_warn("%s:%u: @reflect is not above a struct, union or enum; ignored.", path,
        ...
        if (!(cursor < lexer.tokens->length && lexer.tokens->items[cursor].type == NYA_TOKEN_SYMBOL &&
              lexer.tokens->items[cursor].symbol == '{')) {
            nya_warn("%s:%u: @reflect on a declaration with no body; ignored.", path, lexer.tokens->items[index].line_number);
            continue;
        }
        ...
        if (decl.name[0] == '\0') {
            nya_warn("%s:%u: @reflect on an anonymous type, which nothing could reference; ignored.", path,
                     lexer.tokens->items[index].line_number);
            continue;
        }

        if (_nya_reflect_is_known(set, decl.name)) {
            nya_warn("%s: '%s' is annotated more than once; the later one is ignored.", path, decl.name);
            continue;
        }
    ...
    for (u32 i = 0; i < decl->field_count; i++) {
        ...
        if (field->array_extent[0] != '\0') {
            ...
            if (element != nullptr) {
                ...
                nya_warn("%s: '%s.%s' has undescribed element type '%s'; skipped. Add @reflect to it, or @skip to the field.",
        ...
        if (symbol == nullptr) {
            ...
            // editor days later.
            if (field->pointer_depth == 0) {
                nya_warn("%s: '%s.%s' has undescribed type '%s'; skipped. Add @reflect to it, or @skip to the field.",
                         decl->source_file, decl->name, field->name, field->type_spelling);
            }
