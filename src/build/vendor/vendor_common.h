...
/* The compiler cache sits in front of the vendors only: they are hundreds of small -O2 files a fresh checkout or worktree rebuilds, while the unity-build engine is one translation unit whose preprocess costs nearly as much as a cache hit saves (185 ms of 2750, and worsening as it grows). So the engine, tests and examples call the compiler directly; the vendors go through the cache. VENDOR_CC is the program to run and VENDOR_CC_LAUNCHED the prefix in front of the arguments: "sccache"/"clang" with a cache, "clang"/"" without, the empty one dropped by the command runner. */
