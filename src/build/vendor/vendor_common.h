...
// For GetSystemInfo below, and after nyangine.h because that is what defines OS_WINDOWS. Included here rather than relied upon: nothing in nyangine.h pulls windows.h in, only the platform .c files do, so this header used to compile on a Windows host purely because build.c included nyangine.c ahead of build.h. It no longer does, and a header should not need its includer to have gone first anyway.
#if OS_WINDOWS
#include <windows.h>
#endif
...
/* The cache in front of the compiler, for the vendors and for nothing else. Only the vendors, deliberately. A cache decides it has seen a compile before by hashing what the compiler would see, and sccache does that by preprocessing the translation unit: this engine is a unity build, so one translation unit is the whole of it, and preprocessing that costs nearly as much as compiling it — measured, a hit on the engine saved 185 ms of 2750. It gets worse as the engine grows, and a miss is the preprocess on top of the compile rather than instead of it, so the cache would eventually cost more than it saves on exactly the translation units that are growing. The vendors are the opposite shape, hundreds of small files at -O2, and they are what a fresh checkout or a worktree rebuilds from nothing. So the engine, the tests and the examples call the compiler directly and the vendors go through the cache. Runtime strings because whether there is a cache is a property of the machine, and taking their address is still a compile time constant, so they sit inside the static vendor rules below. VENDOR_CC is the program to run and VENDOR_CC_LAUNCHED goes in front of the arguments: with a cache that is "sccache" and "clang", without one "clang" and "", which the command runner drops. */
...
NYA_INTERNAL void nya_vendor_detect_compiler_cache(void) {
    /* sccache first because it is the one that can be central: its storage is a directory, an S3 bucket or a Redis, so every worktree on this machine and a CI runner share one cache, and a vendor set built once is built for all of them. ccache second, which caches per machine. */
    ...
    for (u32 index = 0; index < nya_carray_length(CACHES); index++) {
        ...
        // asking it for its version rather than looking for the file: a binary that is there but cannot run would otherwise fail every compile with a message about the cache, not the code.
...
/* Valve ships the Steamworks SDK prebuilt, so there is nothing to build and no file of its own to put it in: a Steam target links the library and carries it beside the executable. The plugin declares the flat C functions it calls, so no header is included either. */
