...
#define _NYA_OS_PAGE_MINCORE_BATCH 1024
...
void* nya_os_page_reserve(u64 size) {
    ...
    // no access and no swap reservation: nothing is charged until nya_os_page_commit.
...
u64 nya_os_page_resident_bytes(const void* address, u64 size) {
    ...
    for (u64 at = start; at < end;) {
        ...
        u64 pages = left < _NYA_OS_PAGE_MINCORE_BATCH ? left : _NYA_OS_PAGE_MINCORE_BATCH;

        unsigned char in_core[_NYA_OS_PAGE_MINCORE_BATCH];
...
u64 nya_os_process_resident_bytes(void) {
    ...
    // the first field is the total size; the second is the resident set, so the first is stepped over.
    char* cursor = nullptr;
    (void)strtoull(line, &cursor, 10);

    unsigned long long resident_pages = strtoull(cursor, nullptr, 10);

    return (u64)resident_pages * nya_os_page_size();
}
