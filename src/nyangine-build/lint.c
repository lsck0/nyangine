...
NYA_INTERNAL const NYA_ConstCString _LINT_ROOTS[] = {
    "./src/nyangine-std", "./src/nyangine-core", "./src/nyangine-ui", "./src/nyangine-plugins", "./src/gnyame", "./src/nyangine-build", "./src/genyarated", "./examples", "./tests", "./bench",
};
 ...
 * exactly where src/nyangine-build/pp/reflection.c looks for an annotation. Written to agree with it: a rule
...
#include "nyangine-build/lint_allowances.h"
...
void _lint_rule_layering(Lint* lint) {
    ...
    for (u32 edge = 0; edge < nya_carray_length(_LINT_LAYERING_ALLOWED); edge++) {
        ...
        _lint_report(lint, "layering", nya_string_from(nya_arena_global, "./src/nyangine-build/lint_allowances.h"), 1,
...
void _lint_rule_verb_pairs(Lint* lint) {
    ...
    for (u32 entry = 0; entry < nya_carray_length(_LINT_VERB_PAIRS_ALLOWED); entry++) {
        ...
        _lint_report(lint, "verb-pair", nya_string_from(nya_arena_global, "./src/nyangine-build/lint_allowances.h"), 1,
...
void _lint_rule_callers(Lint* lint) {
    ...
    for (u32 entry = 0; entry < nya_carray_length(_LINT_CALLERS_ALLOWED); entry++) {
        ...
        _lint_report(lint, "caller", nya_string_from(nya_arena_global, "./src/nyangine-build/lint_allowances.h"), 1,
