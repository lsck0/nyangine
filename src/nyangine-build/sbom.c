...
#include "nyangine-build/vendor/licence_allowlist.h"
...
void sbom_runner(NYA_ArgCommand* command) {
    ...
    for (u32 i = 0; i < count; i++) {
        ...
        nya_log_error("%s is under '%s', which is not on the licence allowlist (see src/nyangine-build/vendor/licence_allowlist.h).", deps[i].name,
                      deps[i].licence);
        rejected++;
    }
    if (rejected > 0) {
