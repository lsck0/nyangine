...
void new_runner(NYA_ArgCommand* command) {
    ...
    if (kind == NEW_KIND_APP) {
        ...
        nya_log_info("  1. src/nyangine-build/flags.h: an APP_%s_NAME / APP_%s_DLL_SOURCE pair and the debug binary/dll names,", upper, upper);
        nya_log_info("     beside APP_GNYAME_CLI_NAME (see the APPS section).");
        nya_log_info("  2. src/nyangine-build/on_linux/build_linux.h: the host, compile-dll, build-dll and metarule rules,");
        nya_log_info("     modelled on build_gnyame_cli_debug_* .");
        nya_log_info("  3. src/nyangine-build/cli.c: a `run %s` and a `build %s` subcommand naming those rules.", name, name);
