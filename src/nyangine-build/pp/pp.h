...
#include "nyangine-build/pp/stale.h"
/**/
#include "nyangine-build/pp/asset.h"
#include "nyangine-build/pp/cheatsheet.h"
#include "nyangine-build/pp/i18n.h"
#include "nyangine-build/pp/lambda.h"
#include "nyangine-build/pp/luabind.h"
#include "nyangine-build/pp/reflection.h"
#include "nyangine-build/pp/watch.h"
...
NYA_INTERNAL NYA_BuildRule index_assets = {
    .name             = "index_assets",
    .is_metarule      = true,
    // These generators all write *source* the compile rules then consume, so everything that compiles depends on them and they share this ordering, not any tie to asset indexing: generate_reflection and generate_strings emit source; generate_lua_bindings the C that lua_engine.c includes; generate_lambdas and generate_watches the headers a body and its generated form must land in one build with; generate_cheatsheet regenerates here so a header edit and its reference ship together.
