...
NYA_INTERNAL NYA_ConstCString _NYA_UI_HTML_PAGE =
    ...
    "  html,body{margin:0;height:100%%;background:var(--bg);color:var(--ink);font:14px/1.3 ui-monospace,SFMono-Regular,Menlo,monospace}\n"
    "  #nya-surface{position:relative;width:100%%;height:100vh;overflow:hidden}\n"
    "  #nya-surface>*{position:absolute;box-sizing:border-box}\n"
    "  .nya-panel{background:var(--panel);border:1px solid var(--line);border-radius:6px}\n"
    "  .nya-label{color:var(--ink);white-space:nowrap;display:flex;align-items:center}\n"
