...
/*
 * The filesystem reader is compiled into *every* build, not only the filesystem backend.
 *
 * An external asset is one that did not exist when the game was built, so there is nothing in the
 * blob to look up and it has to come off disk no matter which backend this build normally uses.
 * Without this a dropped file would work in development and crash in release.
 */
...
/** Picks the source: external assets always come off disk, the rest follow the build's backend. */
NYA_INTERNAL void _nya_asset_load_raw(NYA_AssetHandle handle, b8 external, OUT NYA_Asset* out_asset);
...
#else
    nya_unused(modification_time); // nothing watches for changes in a build with a baked blob
#endif
}
...
NYA_INTERNAL void _nya_asset_load_raw(NYA_AssetHandle handle, b8 external, OUT NYA_Asset* out_asset) {
    ...
    if (external) {
        _nya_asset_load_raw_from_filesystem(handle, out_asset);
        return;
    }

#ifdef NYA_ASSET_BACKEND_BLOB
    _nya_asset_load_raw_from_blob(handle, out_asset);
#else
    _nya_asset_load_raw_from_filesystem(handle, out_asset);
#endif
}
...
NYA_INTERNAL void _nya_asset_unload_raw(NYA_Asset* asset) {
    ...
    // something that was read off disk owns its memory.
    if (asset->load_parameters.external) {
    ...
    switch (asset->type) {
        ...
        case NYA_ASSET_TYPE_GRAPHICS_PIPELINE: {
            ...
            if (result.kind == NYA_ERROR_NONE) {
                result = nya_filesystem_last_modified(
                    asset->load_parameters.as_graphics_pipeline.fragment_shader_handle,
                    &fragment_shader_modification_time
                );
            }
...
void _nya_asset_loading_process(NYA_Event* event) {
    ...
    nya_array_foreach (system->loading_queue, parameters) {
        ...
        switch (parameters->type) {
            case NYA_ASSET_TYPE_TEXT: {
                _nya_asset_load_raw(parameters->handle, parameters->external, asset);
