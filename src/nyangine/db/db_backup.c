...
// This file is compiled as part of db.c, after db_sql.c: it reaches into struct NYA_Database and
// calls _nya_sql_kind_from_sqlite, both private to db_sql.c and both in scope here because the two
// are one translation unit. See db.c for the include order that guarantees it.
...
NYA_Error nya_sql_backup_with_options(NYA_Database* source, NYA_SqlBackupOptions options) {
    ...
    // A key, if given, is validated and refused on a cipher-less build exactly as nya_sql_open does,
    // and before any file is made — the same reasoning: a copy asked for encrypted must never land in
    // the clear instead.
    ...
    // A copy of an encrypted source with no key would be written in the clear, which is a quieter way
    // to lose the encryption than refusing is.
    ...
    // Key the destination before the first page is copied, or the copy is written in the clear. Only
    // reachable on a build with a cipher; a key on any other build was refused above.
    ...
    // "main" on both sides: the primary schema of each connection. sqlite3_backup_init reports its
    // failure on the destination handle rather than through a return code.
    ...
    // A step that finds the source locked by another writer returns BUSY or LOCKED rather than
    // failing outright; sleep briefly and try again, bounded so a wedged writer can never hang the
    // backup forever. A write that lands between steps is noticed by the API and its pages recopied,
    // which is what makes the finished copy a consistent snapshot rather than a smeared one.
    ...
    // finish frees the backup object on every path, so it is always called. It returns the last error
    // the copy met, or SQLITE_OK when every page made it across.
    ...
    // close_v2, matching nya_sql_close: it tolerates a connection with work still pending rather than
    // refusing, though a finished backup leaves none.
...
NYA_Error nya_sql_vacuum_into(NYA_Database* database, NYA_ConstCString dest_path) {
    ...
    // The same no-clobber rule as nya_sql_backup. VACUUM INTO refuses an existing file itself, but
    // failing here keeps the error one the caller can read rather than SQLite's phrasing.
    if (nya_filesystem_exists(dest_path)) {
        return nya_error(NYA_ERROR_INVALID_ARGUMENT, "'%s' already exists; a backup will not overwrite it", dest_path);
    }

    // Bound, not interpolated: VACUUM INTO takes its target as an ordinary expression, so the path is
    // data and a quote in it cannot become syntax.
...
NYA_Error nya_sql_checkpoint(NYA_Database* database, NYA_SqlCheckpoint mode) {
    ...
    // A fixed string per mode rather than an interpolated one: the mode is a closed set, so there is
    // nothing to bind and nothing a caller could inject.
    ...
    // The pragma returns one row of counters (busy, log, checkpointed); exec discards it. A checkpoint
    // that a live reader blocks reports busy in that row rather than an error code, so exec sees
    // success either way, which matches the documented "does what it can and returns" behaviour.
