...
NYA_Error nya_command_run(NYA_Command* command) {
    // Spawn then wait, so there is one implementation of each half and no way for the blocking and non-blocking paths to drift apart.
...
NYA_Error nya_command_spawn(NYA_Command* command) {
    ...
    // Both arrays are handed on as they are, terminated by the null the last slot still holds; a caller that fills every slot has lost the terminator, not just the room.
    ...
    // argv, with the program in front as the child's own name; bounded by the arguments array plus that name and the terminator.
    NYA_ConstCString argv[NYA_COMMAND_MAX_ARGUMENTS + 2] = { nullptr };
    u32              argc                                = 0;

    argv[argc++] = command->program;
    // An empty argument is dropped rather than passed on: it lets a static rule carry a sometimes-there prefix (a compiler cache) without two copies of the rule.
    ...
    // The child has its own copies of the write ends now, and these must go: a read end whose writer is still open here never reaches end of file, and the drain would wait forever.
    nya_os_pipe_close(stdout_write);
    ...
    /* The read ends are handed to nya_command_wait, which drains and closes them; held open until then, since a child writing past a pipe buffer blocks until someone reads. */
...
void nya_command_wait_ready(NYA_Command* const* commands, u32 count, u32 timeout_ms) {
    ...
    // Both halves, because the two targets wake on different things (one on the processes, one on the pipes); with neither left it is the plain sleep a child between closing output and exiting needs.
...
NYA_INTERNAL void _nya_command_drain(NYA_Command* command, b8 block) {
    ...
    // Both pipes every pass, neither drained to EOF before the other is looked at: a child filling one while the reader waits on the other can never exit; the drain ends when both close.
    for (u32 reads = 0; block || reads < _NYA_COMMAND_DRAIN_MAX_READS; reads++) {
        ...
        for (u32 i = 0; i < 2; i++) {
            ...
            if (status == NYA_OS_PROCESS_OK) {
                // The length-carrying overload, because captured output may contain zero bytes and the cstring one would stop at the first.
                nya_string_extend(targets[i], &(NYA_String){ .length = taken, .items = buffer });
                made_progress = true;
                continue;
            }

            // End of file, or a read that will not return: nothing better than to stop reading; the child sees a closed pipe and the wait still reaps it.
            nya_os_pipe_close(*pipes[i]);
            *pipes[i]     = NYA_OS_PIPE_NONE;
            made_progress = true;
        }
...
NYA_INTERNAL void _nya_command_finish(NYA_Command* command, s32 exit_code) {
    ...
    // Cleared so a second wait on the same command asserts rather than reaping an unrelated process that reused the pid, or waiting on a handle already closed.
    command->process = (NYA_OsProcess){ 0 };
}

NYA_INTERNAL u64 _nya_command_now_ms(void) {
    // The os clock, not nya_clock_get_monotonic_ms, which is the same number through a module above this; the division platform/clock does is written here so base calls only downwards.
