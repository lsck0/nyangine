...
b8 nya_type_parse_guess(const u8* data, u64 length, OUT NYA_Type* out_type, OUT void* out_value) {
    nya_unused(data, length, out_type, out_value);
    nya_todo();
}
...
NYA_INTERNAL b8 _nya_type_try_parse_bool(const u8* data, u64 length, OUT u128* out_value) {
    nya_unused(data, length, out_value);
    nya_todo();
}
