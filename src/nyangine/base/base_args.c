    ...
    for (s32 arg_index = start_index; arg_index < argc; arg_index++) {
        ...
        end_parameter_search:
            ...
            if (param->value.type == NYA_TYPE_B8) {
                ...
                if (!flag_value_str || nya_string_starts_with(flag_value_str, "--")) {
                    param->value.as_b8 = true;
                } else if (_nya_args_parse_value(&param->value, flag_value_str)) {
                    arg_index++;
                } else {
                    return nya_error(NYA_ERROR_NOT_OK, "Failed to parse boolean value for flag '--%s': '%s'.", param->name, flag_value_str);
                }
            } else {
                // integer, float and string flags require a value
                if (arg_index + 1 >= argc) { return nya_error(NYA_ERROR_NOT_OK, "Flag '--%s' expects a value, but none was provided.", param->name); }

                NYA_CString flag_value_str = argv[arg_index + 1];
