 ...
 * formats, or decides what an error means belongs in `base` (nya_filesystem_*, NYA_File) or in
 * `platform` (nya_clock_*, the terminal, ipc, signals, the host's own description) instead.
 *
 ...
 * where it used to be ~30 functions written twice, and platform/clock/clock.c, which is seven
 * functions over two os calls where it used to be seven functions twice.
 * */
#pragma once
