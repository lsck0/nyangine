...
- **Web client / game→web (CSR).** `[~]` The renderer is 100% SDL_GPU with no browser backend — the wall that blocks a rendered game on the web. The non-renderer half of `platform/web` is now seeded (2026-09-24): `web_clock` (performance.now/Date.now), `web_random` (crypto.getRandomValues), `web_storage` (localStorage), `web_fetch` + `web_socket` (polled async seams driven from the frame loop, no ASYNCIFY), each `#if OS_WASM` with a native fallback and proven under `node`. Still needs the **WebGL2/WebGPU renderer backend** and browser input. UI-only CSR runs today; a rendered game does not.
