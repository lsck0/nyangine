...
- **Tor / onion-service deploy.** `[ ]` (User, 2026-09-24.) A hidden-service deploy profile (a `tor` HiddenService beside the scratch image), a signed **mirror-attestation** endpoint so a mirror can be verified against the origin's key, and a proof-of-work / captcha layer for abuse resistance. Blocked on agent spawns (session 429) as of 2026-09-24.
- **Smaller open items:** pentest pass over `http_server`; scheduled CI (fuzz/simulation/benchmarks); clang-format gate; job queue; passkeys (WebAuthn); DOM presenter + shadcn-like widget set + node/code editors; a parsed-newtype helper; typed route-table client calls; web hot reload.
