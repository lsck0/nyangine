...
- **Smaller open items:** wrap TLS into the http listener; live-CA ACME run; clang-format gate; a code-editor widget; a devenv/nix toolchain + CBMC model-checking + a commit-msg conventional-commit hook + a `nya new` scaffolder (see [[rust-template-learnings]]).
