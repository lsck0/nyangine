...
- **Layering: a server links the whole engine.** `[~]` Compile seam now split (2026-09-24): `net`/`http` carry no `core`/renderer/SDL include, `core_event` is SDL-free, so `nyangine.h` pulls net/http under `#if !defined(NYA_NO_SDL) || defined(NYA_SERVER)`. **`-DNYA_NO_SDL -DNYA_SERVER` compiles http/net/crypto/tls/db/accounts with no core or renderer** — the thing proven impossible earlier that day. Every existing build is unchanged (host tool + wasm keep `NYA_NO_SDL` alone → no net). Open: the headless *link* — a server binary needs its own loop ticking `nya_http_server_tick` (today `core_app.c` does it) + the `NYA_SERVER_VENDORS` set; and `NYA_App` still embeds the renderer by value (blocks anything calling `nya_app_get` headless). `--server` (`--gc-sections`) remains the deploy mitigation until the link lands.
...
- **Accounts.** `[~]` Full login flow in `examples/accounts_api` (Argon2id, user store, register/login/TOTP/logout, revocable `__Host-session` cookie, PGP seam), plus **passkeys/WebAuthn** as a factor (`a4749383`, Ed25519, single-use challenges). Open: lift from the example into reusable `accounts` routes; passwordless-primary.
...
- **Renderer.** `[~]` SSR + SSAO post passes and a **GPU compute pipeline** + a compute particle field (`93f86ef3`, desktop-only) landed. Open: more compute effects (raymarched volumes, GPU fluids).
