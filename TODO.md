...
- **TLS in process + ACME.** `[~]` The `tls` module now does both server AND verified **client** sessions (`cc795de7`, SNI + `SSL_set1_host` + trust store) — SMTP rides it. Open: wrap TLS into `http`'s own listener (currently the server takes cert/key but termination is still commonly a proxy's job), and **ACME** so one executable renews its own cert.
...
- **Renderer.** `[~]` Screen-space reflections (`4b5685c2`) and classic SSAO (`c0a52980`) landed as opt-in web-safe post passes; planar water reflection + the composed showcase scene already in. Open: true GPU **compute passes** (raymarched volumes, GPU fluids/particles) — needs a compute-pipeline abstraction, not just fragment passes.
- **Model/SO/DTO web profile.** `[~]` `@secret` encrypted serde fields landed (`77a4539b`, XChaCha20-Poly1305, key-seam). Open: the Model/SO/DTO conversion split in `http`/`db` and the `web` profile refusing model/SO headers.
- **Smaller open items:** pentest pass over `http_server` (2-cookie IDOR/BFLA); scheduled CI (fuzz/simulation/benchmarks); clang-format gate; node/code-editor widgets; ACME.
