...
s32 main(void) {
    ...
    // Static rather than on the stack: NYA_TextRun is about thirty kilobytes, which render_text.h says is why a caller keeps one and reuses it. A benchmark that allocated one per iteration would be measuring the memset.
    ...
    // one short line, what a call site asks for. If twenty of these match the figure above, batching gains nothing and only memoising is left.
    nya_bench("one hud line", nya_utf8_count(hud_lines[0]), {
        nya_bench_keep(nya_text_shape(hud, hud_lines[0], 0, 0, &run));
    });

    // Measuring without keeping the glyphs, which is the *other* half of what a frame does: every centred label and every panel sized to its text measures before it draws, so a HUD pays this on top of the shaping above rather than instead of it.
    ...
    // Wrapped, where the shaper breaks the lines. Line breaking is the part that is more work than the old codepoint walk did, rather than the same work done properly.
    ...
    /* The same cases through nya_text_shape_with_font once the first call has filled the cache: the handle, the lookup, and copying the laid out text into the run. */
    nya_bench("hud frame, 20 lines", hud_glyphs, {
        for (u32 i = 0; i < hud_line_count; i++) {
