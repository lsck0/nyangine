...
s32 main(void) {
    ...
    /*
     * Built as NYA_PERMISSION_SYSTEM, the actor that outranks everyone and holds every bit, so the
     * setup is not itself a test of the hierarchy rules. What is measured below is resolution, not
     * the gated writes that fill the table.
     */
    ...
    /*
     * Every subject holds member; a third also moderate and a tenth administer, which is roughly the
     * shape of a real membership and enough held roles that the union in step one is not trivial.
     */
    ...
    // The one call a route makes: does this subject hold this bit on this room. Swept across every
    // subject and every room so the sample is not one lucky cache line.
    ...
    // The whole resolved set, which a screen that greys out what a subject cannot do asks for once
    // and then reads bit by bit. Same sweep, so the two numbers sit beside each other.
    ...
    // Resource zero: no room in particular, which resolves the held roles alone and skips every
    // overwrite lookup. The floor a guild-wide check pays.
