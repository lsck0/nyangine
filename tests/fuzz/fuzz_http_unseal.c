...
static void fuzz_once(const u8* data, u64 size) {
    ...
    // The bytes are the token. A label the server would actually use; a wrong label is one more reason to
    // refuse and is covered by the property tests, not needed to reach the decode and decrypt here.
    ...
    // An open never claims more bytes than the buffer holds, and — the whole promise of a seal — opening
    // the identical bytes a second time gives the identical plaintext, byte for byte.
    ...
    // The same token under a different label must not open: the label is authenticated, so a token that
    // opened in one slot cannot open in another.
