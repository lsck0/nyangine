...
static void fuzz_once(const u8* data, u64 size) {
    ...
    // The parser walks a C string, so the input is copied and terminated. An embedded NUL simply ends the
    // string early, which the strlen/strncmp/strrchr walk must handle without stepping past it.
    char* encoded = nya_arena_alloc(arena, size + 1);
    if (size > 0) nya_memcpy(encoded, data, size);
    encoded[size] = '\0';

    // The hand-written cost reader: total, and when it accepts, three non-zero parameters that each fit a
    // u32. A misread here is a cost this process would then hash at.
    ...
    // needs_rehash is the public door onto the same parser, and must be total for any column bytes: a hash
    // it cannot read is one worth replacing, never a crash. It reads the fixed-width column, so the input
    // is placed there truncated the way a real row would be.
    ...
    // Verify reaches the deeper parser — the salt and hash base64url decode — and returns false long
    // before the expensive Argon2id whenever the shape is wrong, which for arbitrary bytes is always. It
    // must refuse cleanly rather than read past a field it could not find.
