/**
 * The culling render3d.c runs before queueing a draw: the frustum planes a view-projection yields, which
 * spheres they reject, and when the occlusion buffer gets a say.
 **/
...
/** Resets the batch and builds its frustum from `view_projection`. */
...
s32 main(void) {
    ...
    // ── Inside, and outside each plane alone.
    {
        ...
        nya_check(_nya_render3d_visible(&batch, (f32x3){ 0, 0, -10 }, 1.0F), "a sphere straight ahead is visible");
        ...
        for (u32 i = 0; i < nya_carray_length(outside); i++) {
            ...
            nya_check(!_nya_render3d_visible(&batch, outside[i].center, outside[i].radius), "a sphere beyond %s is culled", outside[i].name);
    ...
    // ── A sphere whose centre is outside but whose edge reaches in is kept; the test is `distance < -radius`.
    {
        ...
        for (u32 plane = 0; plane < 6; plane++) {
            ...
            nya_check(_nya_render3d_visible(&batch, straddle, 1.0F), "plane %u: half a unit out with radius 1 is visible", plane);
            nya_check(!_nya_render3d_visible(&batch, straddle, 0.25F), "plane %u: with radius 0.25 it is culled", plane);
        }

        // a sphere around the camera reaches every plane.
        nya_check(_nya_render3d_visible(&batch, (f32x3){ 0, 0, 50 }, 200.0F), "a huge sphere behind the camera still overlaps the view");
    }
    ...
    // ── Conservative at a corner: outside the view, but not wholly outside any one plane, so kept.
    {
        ...
        nya_check(_nya_render3d_visible(&batch, corner, 1.0F), "and kept, which is the safe direction to be wrong in");
    }
    ...
    // ── A degenerate matrix leaves its planes alone rather than dividing by zero, and culls nothing.
    {
        ...
        nya_check(_nya_render3d_visible(&batch, (f32x3){ 1e6F, -1e6F, 1e6F }, 0.0F), "anything is visible");
    }

    // ── The occlusion buffer is asked only about what survived the frustum, and never in a shadow pass.
    {
        frustum_from(perspective_matrix());

        nya_occlusion_begin(&occlusion, batch.view_projection);
        (void)nya_occlusion_quad(&occlusion, (f32x3){ -40, -40, -10 }, (f32x3){ 40, -40, -10 }, (f32x3){ 40, 40, -10 }, (f32x3){ -40, 40, -10 });

        nya_check(_nya_render3d_visible(&batch, (f32x3){ 0, 0, -20 }, 1.0F), "without a buffer set, behind the wall is visible");
        nya_check(batch.frame_occluded == 0, "and nothing counts as occluded");

        batch.occlusion = &occlusion;

        nya_check(!_nya_render3d_visible(&batch, (f32x3){ 0, 0, -20 }, 1.0F), "with it set, behind the wall is hidden");
        nya_check(batch.frame_occluded == 1, "and counted, got %u", batch.frame_occluded);

        nya_check(_nya_render3d_visible(&batch, (f32x3){ 0, 0, -5 }, 1.0F), "in front of the wall is visible");

        u32 tests_before = nya_occlusion_stats(&occlusion).tests;
        nya_check(!_nya_render3d_visible(&batch, (f32x3){ -100, 0, -20 }, 1.0F), "outside the frustum and behind the wall is culled");
        ...
        nya_check(_nya_render3d_visible(&batch, (f32x3){ 0, 0, -20 }, 1.0F), "a shadow pass ignores the camera's buffer");
        nya_check(!_nya_render3d_visible(&batch, (f32x3){ -100, 0, -20 }, 1.0F), "but still culls by the frustum");
        nya_check(batch.frame_occluded == 1, "and counts nothing, got %u", batch.frame_occluded);
    }
