...
s32 main(void) {
    ...
    {
        u64 size    = 64 * page;
        u8* address = nya_memory_reserve(size);
        nya_check(address != nullptr, "reserving 64 pages should succeed");
        defer nya_memory_release(address, size);

        nya_check(nya_memory_commit(address, 4 * page), "committing the first four pages should succeed");
        nya_check(nya_memory_resident_bytes(address, size) == 0, "committed but unwritten pages should not be resident");
        ...
        nya_check(nya_memory_resident_bytes(address, size) == 2 * page, "two written pages should be resident, got " FMTu64,
                  nya_memory_resident_bytes(address, size));

        // an unaligned range still counts the pages it touches.
        nya_check(nya_memory_resident_bytes(address + 1, 10) == page, "a range inside one page should count that page");

        // a later commit extends the usable range without disturbing what was written.
        nya_check(nya_memory_commit(address + (4 * page) + 1, page), "a commit starting mid page should round out");
        address[5 * page - 1] = 7;
    ...
    // ── The process total is at least what this test has resident on its own.
    {
        u64 size    = 32 * page;
        u8* address = nya_memory_reserve(size);
        nya_check(address != nullptr && nya_memory_commit(address, size), "reserving and committing 32 pages should succeed");
        defer nya_memory_release(address, size);

        nya_memset(address, 1, size);

        u64 process = nya_memory_process_resident_bytes();
    ...
    nya_check(nya_memory_resident_bytes(nullptr, page) == 0, "null should measure as nothing");
    nya_check(nya_memory_resident_bytes(&page, 0) == 0, "an empty range should measure as nothing");
    nya_memory_release(nullptr, page);
