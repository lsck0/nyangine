...
s32 main(void) {
  ...
  // TEST: negatives are rejected by the unsigned casts The check is `val >= MIN && val <= MAX`. When MIN and MAX are unsigned and val is signed, the usual arithmetic conversions turn a negative val into a very large unsigned one before either comparison happens. For the narrow types the upper bound still catches it. For u64 there is no value large enough to exceed U64_MAX, so both halves are satisfied and -1 casts silently.
  ...
  // TEST: the argument is evaluated exactly once These are statement-expression macros that name `val` more than once. If it is not bound to a temporary first, an argument with a side effect happens as many times as it is written, which is the classic macro trap and silently corrupts a counter at the call site.
