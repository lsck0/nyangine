...
s32 main(void) {
  ...
  nya_hmap_add(get_map, 5U, (u64)500);
  ...
  nya_hmap_add(remove_map, 1U, (u64)10);
  nya_hmap_add(remove_map, 2U, (u64)20);
  nya_hmap_add(remove_map, 3U, (u64)30);
  ...
  nya_assert(*nya_hmap_get(remove_map, 1U) == (u64)10);
  nya_assert(*nya_hmap_get(remove_map, 3U) == (u64)30);
  ...
  nya_hmap_add(remove_first_last, 100U, (u64)1);
  nya_hmap_add(remove_first_last, 200U, (u64)2);
  nya_hmap_add(remove_first_last, 300U, (u64)3);
  ...
  nya_hmap_add(clear_map, 1U, (u64)1);
  nya_hmap_add(clear_map, 2U, (u64)2);
  nya_hmap_add(clear_map, 3U, (u64)3);
  ...
  for (u32 i = 0; i < 10; ++i) { nya_hmap_add(resize_map, i, (u64)(i * 100)); }
  ...
  for (u32 i = 0; i < 100; ++i) { nya_hmap_add(stress_map, i, (u64)(i * 10)); }
  ...
  nya_hmap_add(orig_map, 1U, (u64)100);
  nya_hmap_add(orig_map, 2U, (u64)200);
  nya_hmap_add(orig_map, 3U, (u64)300);
  ...
  nya_assert(*nya_hmap_get(copy_map, 1U) == (u64)100);
  nya_assert(*nya_hmap_get(copy_map, 2U) == (u64)200);
  nya_assert(*nya_hmap_get(copy_map, 3U) == (u64)300);
  ...
  // Note: copy_map points to a stack-allocated copy, so no destroy needed

  // TEST: nya_hmap_move
  NYA_Arena*    arena2   = nya_arena_create(.name = "test_hmap_move");
  NYA_HMapᐸu32ˏu64ᐳ* move_map = nya_hmap_create(arena, u32, u64);
  nya_hmap_add(move_map, 10U, (u64)1000);
  nya_hmap_add(move_map, 20U, (u64)2000);

  nya_hmap_move(move_map, arena2);
  nya_assert(move_map->arena == arena2);
  nya_assert(move_map->length == 2);
  nya_assert(*nya_hmap_get(move_map, 10U) == (u64)1000);
  nya_assert(*nya_hmap_get(move_map, 20U) == (u64)2000);
  ...
  nya_hmap_add(iter_map, 1U, (u64)100);
  nya_hmap_add(iter_map, 2U, (u64)200);
  nya_hmap_add(iter_map, 3U, (u64)300);
  ...
  nya_hmap_add(signed_map, -5, 50);
  nya_hmap_add(signed_map, 0, 0);
  nya_hmap_add(signed_map, 5, -50);
  ...
  nya_hmap_add(remove_nonexist, 1U, (u64)10);
  ...
  nya_hmap_add(destroy_map, 1U, (u64)1);
  nya_hmap_add(destroy_map, 2U, (u64)2);
  ...
  nya_hmap_add(update_map, 1U, (u64)100);
  nya_assert(*nya_hmap_get(update_map, 1U) == (u64)100);
  nya_hmap_add(update_map, 1U, (u64)999);
  nya_assert(update_map->length == 1); // should still be 1, not 2
  nya_assert(*nya_hmap_get(update_map, 1U) == (u64)999);
  nya_hmap_destroy(update_map);

  // TEST: zero key value
  NYA_HMapᐸu32ˏu64ᐳ* zero_map = nya_hmap_create(arena, u32, u64);
  nya_hmap_add(zero_map, 0U, (u64)0);
  nya_assert(zero_map->length == 1);
  nya_assert(nya_hmap_contains(zero_map, 0U) == true);
  nya_assert(*nya_hmap_get(zero_map, 0U) == (u64)0);
  ...
  for (u32 i = 0; i < 20; ++i) { nya_hmap_add(collision_map, i, (u64)(i * 100)); }
  ...
  nya_hmap_add(reuse_map, 1U, (u64)1);
  nya_hmap_add(reuse_map, 2U, (u64)2);
  nya_hmap_clear(reuse_map);
  nya_assert(reuse_map->length == 0);
  // Insert same keys again
  nya_hmap_add(reuse_map, 1U, (u64)10);
  nya_hmap_add(reuse_map, 2U, (u64)20);
  nya_assert(reuse_map->length == 2);
  nya_assert(*nya_hmap_get(reuse_map, 1U) == (u64)10);
  nya_assert(*nya_hmap_get(reuse_map, 2U) == (u64)20);
  nya_hmap_destroy(reuse_map);

  // TEST: remove all items one by one
  NYA_HMapᐸu32ˏu64ᐳ* remove_all_map = nya_hmap_create(arena, u32, u64);
  for (u32 i = 0; i < 10; ++i) { nya_hmap_add(remove_all_map, i, (u64)i); }
  for (u32 i = 0; i < 10; ++i) {
  ...
  // Should be reusable
  nya_hmap_add(remove_all_map, 42U, (u64)42);
