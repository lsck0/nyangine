...
s32 main(void) {
  ...
  NYA_Arena* defrag_arena = nya_arena_create(.name = "defrag_arena", .defragmentation_enabled = true, .defragmentation_threshold = 2);
  ...
  // NYA_MemoryActionArray* actions = nya_arena_get_memory_actions(); nya_array_foreach (actions, action) { switch (action->type) { case NYA_MEMORY_ACTION_ALLOC: { printf( "Memory Action - ALLOC: Arena='%s', Function='%s', File='%s', Line=%u, Ptr=%p, Size=" FMTu64 "\n", action->arena_name, action->function_name, action->file_name, action->line_number, action->alloc.ptr, action->alloc.size ); } break; case NYA_MEMORY_ACTION_REALLOC: { printf( "Memory Action - REALLOC: Arena='%s', Function='%s', File='%s', Line=%u, OldPtr=%p, OldSize=" FMTu64 ", NewPtr=%p, NewSize=" FMTu64 " \n", action->arena_name, action->function_name, action->file_name, action->line_number, action->realloc.old_ptr, action->realloc.old_size, action->realloc.new_ptr, action->realloc.new_size ); } break; case NYA_MEMORY_ACTION_FREE: { printf( "Memory Action - FREE: Arena='%s', Function='%s', File='%s', Line=%u, Ptr=%p, Size=" FMTu64 "\n", action->arena_name, action->function_name, action->file_name, action->line_number, action->free.ptr, action->free.size ); } break; case NYA_MEMORY_ACTION_FREE_ALL: { printf( "Memory Action - FREEALL: Arena='%s', Function='%s', File='%s', Line=%u", action->arena_name, action->function_name, action->file_name, action->line_number ); } break; case NYA_MEMORY_ACTION_GARBAGE_COLLECT: { printf( "Memory Action - GARBAGE_COLLECT: Arena='%s', Function='%s', File='%s', Line=%u", action->arena_name, action->function_name, action->file_name, action->line_number ); } break; case NYA_MEMORY_ACTION_ARENA_NEW: { printf( "Memory Action - ARENA_NEW: Arena='%s', Function='%s', File='%s', Line=%u", action->arena_name, action->function_name, action->file_name, action->line_number ); } break; case NYA_MEMORY_ACTION_ARENA_DESTROY: { printf( "Memory Action - ARENA_DESTROY: Arena='%s', Function='%s', File='%s', Line=%u", action->arena_name, action->function_name, action->file_name, action->line_number ); } break; case NYA_MEMORY_ACTION_COPY: { printf( "Memory Action - COPY: Arena='%s', Function='%s', File='%s', Line=%u, Ptr=%p, Size=" FMTu64 ", CopyPtr=%p\n", action->arena_name, action->function_name, action->file_name, action->line_number, action->copy.ptr, action->copy.size, action->copy.copy_ptr ); } break; case NYA_MEMORY_ACTION_MOVE: { printf( "Memory Action - MOVE: SrcArena='%s', Function='%s', File='%s', Line=%u, Ptr=%p, Size=" FMTu64 ", DstArena='%s', DstPtr=%p\n", action->arena_name, action->function_name, action->file_name, action->line_number, action->move.ptr, action->move.size, action->move.target_arena_name, action->move.target_ptr ); } break; default: break; } }
