...
s32 main(void) {
  ...
  test_flag_with_explicit_value();
  test_integer_flag_parameter();
  test_float_flag_parameter();
  test_string_flag_parameter();
  test_multiple_flags();
  test_subcommand();
  test_positional_parameter();
  test_variadic_positional_parameter();
  test_subcommand_validation();
  test_command_path_printing_repeats();
  test_command_path_joining();
  test_walking_command_tree();
  test_shell_registry();
  test_unknown_shell_is_an_error();
  test_generated_zsh_script_covers_whole_tree();
  test_zsh_escaping_of_descriptions();
