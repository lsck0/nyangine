...
NYA_INTERNAL u64 note_count(void) {
    ...
    // A database that cannot be read answers zero rather than a number it made up; the handler that matters answers 500 on its own failure, and the stream would rather be wrong than stop.
...
NYA_INTERNAL void stream_open(NYA_HttpWebSocket* socket) {
    ...
    // Its own failure and nobody else's: a peer whose queue is full is dropped by the server's own pending-write bound, and the loop carries on.
...
NYA_INTERNAL_CALLBACK NYA_HttpStatus notes_query(NYA_HttpExchange* exchange) {
    ...
    /* Decoded by the parser that read the target, so "first+note" and "first%20note" are both "first note". Asked of the target directly: a repeated name or an overlong filter answers 400, where nya_http_request_query_param would read either as no filter and answer with everything. */
    ...
    /* Every row, oldest first, as structs in this exchange's arena. The clause is a literal in this source and the filter below is not part of it: `WHERE text LIKE ?` would read as the obvious thing to write, and it would hand the client a pattern language, since `%` in what it sent is a wildcard. Reading the rows and filtering here keeps "contains" meaning contains. */
    void* rows  = nullptr;
    u32   count = 0;
    ...
    /* In whatever the caller asked for. A nyangine client sends Accept: application/nya-binary (or application/nya for text) and gets the native document; anything else gets JSON. The notes are an NYA_Object rather than a DTO, so the binary answer is untyped. */
...
NYA_INTERNAL NYA_HttpStatus notes_delete(NYA_HttpExchange* exchange) {
    ...
    /* S64, not U64: serde parses every JSON integer as signed, because JSON does not say which it is. Asking for U64 here refused every well formed delete, which is what running it found. */
    ...
    // NOT_FOUND is the database working fine and the row simply not being there, so it counts as a healthy call to the breaker; only a real failure is recorded against it.
    db_record(removed.ok || removed.kind == NYA_ERROR_NOT_FOUND);

    // An id that matches no row is NYA_ERROR_NOT_FOUND, which is a 404 and not a 500: absence is in the return rather than in the data, so the two failures do not have to be told apart here.
...
NYA_INTERNAL NYA_HttpStatus otp_verify(NYA_HttpExchange* exchange) {
    ...
    // the factor proven is what a session is issued against, which is the half of a login this example can honestly do: there is no password and no user store, so the second factor stands for both.
...
/* Data, not code. The OpenAPI document and the /docs page are generated by walking this, so a route that is served and a route that is documented cannot drift apart: there is one of them. `statuses` is the full set each route may answer with. A debug build asserts a handler never returns one it did not declare, which is why the refusals above are listed alongside the successes. */
/* NYA_HTTP_AFFINITY_MAIN on all three: the notes are a static array and two counters with nothing guarding them, so they are read and written where this program's loop is and nowhere else. Making them a worker's would mean a lock around every touch of NOTES, which is a bigger decision than an example should make quietly; the affinity is that decision, written down. */
...
NYA_INTERNAL NYA_HttpRoute NOTE_ROUTES[] = {
    {
     ...
     // .handler_callback is set in main() to nya_callback(notes_query): a reload-safe stand-in for `.handler = notes_query` that the router re-resolves each dispatch, so a code reload never leaves this route pointing into the old image.
     .summary       = "Every note",
     .description   = "A read, and therefore a QUERY rather than a GET. `?contains=text` keeps the notes containing it.",
     // UNAUTHORIZED is here for the optional proof-of-work layer below, which walls the whole notes resource when `--proof-of-work` is set and answers 401 with a challenge; a route's statuses cover its whole chain, and declaring one the chain can produce is safe whether or not the layer is on.
     ...
     },
    {
     ...
     // CONFLICT, and UNPROCESSABLE beyond the handler's own: the idempotency layer on this router can answer 409 for a duplicate still in flight and 422 for a key reused with a different body, and a route's statuses cover its whole chain. BAD_REQUEST covers the layer's malformed-key answer too.
     ...
     },
    {
     ...
     // CONFLICT and UNPROCESSABLE for the same reason as the POST: an unsafe verb behind the idempotency layer can be answered by it, so it declares what the whole chain can produce.
...
/* The second factor's own table. A separate router rather than four more rows above, because these are not a resource: they are three steps of one flow, and a program that wants notes without a factor merges one and not the other. */
/*
...
NYA_INTERNAL NYA_Error mount_bundle(NYA_Arena* scratch) {
    ...
    for (u64 index = 0; index < nya_carray_length(BUNDLE_SOURCES); index++) {
        ...
        // html, css and js are assets like a texture or a font: this reads the file in a debug build and the baked blob in a release, and there is no second pipeline for web files.
    ...
    /* The digest the mirror attestation names, folded over the same files in mount order: each path and its bytes, length-prefixed, so a verifier that fetches these paths from a mirror recomputes exactly this. Done here because this is where the bytes are in hand; the manifest is signed in main(). */
...
NYA_INTERNAL s32 verify_mirror(NYA_ConstCString base_url, NYA_ConstCString pin_b64) {
    ...
    // The key the document carries must be the pinned one: a mirror presenting its own key and a matching signature has signed nothing the verifier asked about, so this is checked before the signature is.
    ...
    // The signature is the origin's. Now prove the mirror actually serves the bytes it names: fetch the same bundle from this host and recompute the digest the manifest carries.
...
NYA_INTERNAL NYA_Error mount_discovery(void) {
    // The sitemap: the pages worth crawling. The API routes are QUERY/POST reads and writes, not pages, so they are not here.
    ...
    // The feed, RSS at /feed.xml and Atom at /atom.xml from one set of items. The description carries an '&', which comes out as an entity — the escaping the feed builders run on every value.
    ...
    // llms.txt, the strict preset: the document states up front that its content is not for training, then points a model at the docs it may read.
...
s32 main(s32 argc, char** argv) {
    ...
    /* What to bind. Empty is the NYA_HttpConfig default, which is loopback only: the safe thing for an example on a laptop, where nothing outside the machine should reach it. A deployment behind nothing — a container whose only job is this server — has to be told to bind the wildcard, which is what `--address 0.0.0.0` is for; the Dockerfile passes exactly that. See NYA_HttpConfig. */
    NYA_ConstCString address = "";

    // A lone flag rather than a pair: --healthcheck turns this binary into a client of itself for one request and then exits, which is what a container's HEALTHCHECK runs. See below.
    b8 health_check = false;

    // Proof of work in front of the notes resource. Off by default so the curl recipes in this file's block still work as written; on, every note request is walled behind a challenge. See http_pow.h.
    ...
    // The attestation origin string and the seed its key is derived from. The seed may also come from the environment, which is where a deployment keeps a secret rather than on the command line.
    ...
    /* The container's liveness probe, and the one an image built FROM scratch can run: no curl in the image, so the binary probes itself. It GETs `/` on the loopback port through the curl client the server already links and exits 0 only on a 200, which is exactly what a Docker HEALTHCHECK wants — a zero or a non-zero. It starts none of the server, so it is cheap and safe to run every few seconds. Pass the same --port the server was given; the address is always loopback here, which the wildcard the server binds includes. See this example's Dockerfile. */
    if (health_check) {
    ...
    /* The other client mode: fetch a mirror's attestation, check it against the pinned origin key, and prove the mirror serves the origin's exact bundle. Runs and exits like the healthcheck, starting none of the server. See verify_mirror above, and the deploy README for the curl form of the same. */
    ...
    /* The notes. Under the save root rather than beside the binary, so it lands where the rest of this program's data does and is created along with its directory. Everything the server knows is in that one file, which is what makes a restart uneventful: stop it, start it, and the notes are still there. It is not encrypted — see the encryption note in db.h — so nothing goes in it here that would matter if somebody read the disk. */
    NYA_Error saves = nya_system_save_init();

    if (!saves.ok) {
        // Fatal here, where it is a warning in a game: a game with no writable directory can still be played, and a server whose whole job is to keep what it is sent cannot run without one.
    ...
    // Creates the table on a first run, and on any later one brings it level with the struct above: add a field and the column appears, and the notes written before it read it as zero. What it will not do is guess at a rename or a drop; see db_migrate.h.
    NYA_EXPECT(nya_orm_schema_migrate(NOTES_TABLE), "while bringing the notes table level with the model");

    /* The database breaker and the two readiness checks it feeds. A handful of consecutive database failures trips the breaker OPEN, and while it is OPEN the "notes-db" readiness check reports not-ready, so /readyz answers 503 and this instance drops out of a load balancer's rotation until the database recovers. The "db" check is the direct one — does the file answer a query now — and the breaker check is the composed one, reflecting a state base_circuit already keeps. See base_circuit.h and http_health.h. */
    ...
    // the secret the sessions are signed with, made here so that nothing ships one and a restart ends every session this server issued.
    ...
    /* The loudest level, on purpose: this example exists to be run and read, and a summary line would show none of what the layer does. Nothing turns the deny list off, and `deny` adds the API key header a reverse proxy in front of a server like this one usually forwards. A program with a config file sets this section in `engine.nya` instead and gets it back on every reload. */
    ...
    /* The idempotency store the notes router's layer shares. Readied here, before the layer can run, so its lock exists for the four workers: a layer runs on whichever worker answers the request, and a store without a lock would race across them. A short TTL, because a client's retries are seconds apart and a key nobody sends again should not hold a slot for long. See http_idempotency.h. */
    ...
    /* The proof-of-work wall, when asked for. Its spent-nonce store is readied here, before the layer can run, so its lock exists for the four workers; and the notes router is pointed at the chain that has the wall outermost. A modest difficulty for a demo — a browser clears it unnoticed, a script pays it every request — and the challenge lives a couple of minutes. See http_pow.h. */
    if (proof_of_work) {
    ...
    /* Reload-safe handlers, in one call: it installs the resolvers that turn a route's `handler_callback` token back into a function on every dispatch, so a handler carried as a token survives a code hot reload where a raw pointer baked into the table would dangle. See core_http_reload.h. With it installed, the notes QUERY route below carries its handler as a token. */
    nya_http_router_reloadable();

    // The reload-safe stand-in for `.handler = notes_query`: a token the router re-resolves each dispatch. Written here rather than in the table because nya_callback needs the registry, which is only up now, and because in a hot-reloading build the macro expands to a call, not a constant.
    ...
    /* The TOTP second factor. Merged here and not inside the engine for the reason the notes are: the secret and the guard belong to whoever owns the account, which is this program. */
    ...
    /* The engine's own metrics, which are reflected DTOs: asked for as application/nya-binary they go out carrying their layout hash, the typed half the notes above cannot show. The server does not merge them on its own: without this line /api/metrics is a 404. */
    ...
    /* Liveness and readiness. /healthz is always 200 while the process can answer; /readyz runs the two checks registered above and answers 503 while the database is down or its breaker is OPEN. What an orchestrator polls to know whether to restart this process and whether to send it traffic. */
    ...
    /* /openapi.json and /docs, generated by walking every merged route table. Opt in rather than automatic: a server that does not want to publish its own shape should not have to unmerge it. */
    ...
    /* The discoverability surface: /sitemap.xml, /feed.xml and /atom.xml, /robots.txt and /llms.txt. Built once from this program's own data and served as static bytes; clear frees them on the way out, after the routes are unmerged. See http_sitemap.h and its siblings. */
    ...
    /* The page. The three files are read and hashed once here; merging puts the routes that built on the server, which is the same two steps every other resource here takes. The scratch arena is only alive for the read, because the mount keeps a copy of its own. */
    ...
    /* The mirror attestation. The bundle is mounted and its digest folded above, so now the origin key is loaded, the manifest — this origin, now, that digest — is signed once, and the route that serves it is merged. A key that cannot be loaded is fatal here: an origin that means to publish an attestation and cannot make one should not come up pretending it did. See http_attestation.h and verify_mirror. */
    if (!origin_key_load(attestation_seed)) return EXIT_FAILURE;

    defer nya_crypto_sign_key_pair_destroy(&ORIGIN_KEY);

    // The origin string names where these bytes are served from, which is what a client pins alongside the key: the .onion under Tor, set with --attestation-origin or WEB_SERVER_ORIGIN. Loopback is the honest default for a laptop, and says plainly that a mirror check against it only means anything on one host.
    ...
    /* The stream. A websocket route is not in the OpenAPI document — the specification describes requests and answers, and this is neither — so it is mounted on its own and documented in the line below and in this file's comment. */
